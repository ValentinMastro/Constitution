...
<div
    ...
    onkeydown={(e) =>
        e.key === "Enter" && (onselect ? onselect(student) : onpin(student.id))}
>
    {#snippet academic()}
        <span class="w-3 text-center font-semibold text-slate-500"
    ...
    {#snippet behavior()}
        {#if student.moteur}
            <span
                class="rounded bg-emerald-100 px-1 text-emerald-700"
                title="Moteur">{student.moteur}</span
            >
        {/if}
        {#if student.perturbateur}
            <span
                class="rounded bg-amber-100 px-1 text-amber-700"
                title="Perturbateur">{student.perturbateur}</span
            >
        {/if}
    {/snippet}

    <div class="flex items-center gap-1">
        {#if problems.length}<span class="text-red-500" aria-label="problème"
                >⚠</span
            >{/if}
        <span
            class="flex min-w-0 flex-1 items-center gap-1 font-medium text-slate-800"
        >
            {#if student.lastName || student.firstName}
                <!-- Seul le NOM se tronque (…) ; le prénom reste toujours visible. -->
                <span class="min-w-0 truncate">{student.lastName}</span>
                {#if student.firstName}<span class="shrink-0"
        ...
        {#if linked}<span class="text-indigo-500" aria-label="liens">🔗</span
            >{/if}
        <!-- A/B/C/D toujours en fin de 1ère ligne. -->
        {@render academic()}
        <!-- Sans option : M/Z aussi sur cette ligne (tout à la fin, inchangé). -->
        {#if !options.length}{@render behavior()}{/if}
    </div>
    {#if options.length}
        <!-- Avec options : 2e ligne = options à gauche, M/Z à la fin. -->
        <div class="flex items-center gap-1">
            <div class="flex min-w-0 flex-1 flex-wrap gap-0.5">
                {#each options as option (option.id)}
                    <span
                        class="rounded border-[0.25px] px-1 leading-tight {optionColor(
                            option.name,
                        )}"
                        title="Option">{option.name}</span
                    >
                {/each}
            </div>
            {@render behavior()}
